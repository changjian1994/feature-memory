...
| --- | --- |
| 打开工作台 | 本文件的“交互入口” |
| 恢复上下文 / 继续已有 feature | `feature-memory/ai-handoff.md`、`feature-memory/index.md`、目标 feature 的 `handoff.md`、`progress.md` |
| 创建新 feature | `references/memory-lifecycle.md`、`references/feature-relationship.md`、`references/templates.md` |
| 创建聚合 feature | `references/memory-lifecycle.md`、`references/feature-relationship.md`、`references/templates.md` |
...
| 归档、交接、记忆过多或上下文恢复成本变高 | `references/memory-lifecycle.md` |
...
- 小改动默认不写 memory；单次任务默认最多更新 1-3 个 memory 文件，除非首次初始化、归档压缩、复杂 debug 或用户明确要求。
- 新 Agent 接手时先读 `feature-memory/ai-handoff.md`；默认只继续读取 `ACTIVE` 和 `VERIFYING` 记忆。
...
1. 先读取 `feature-memory/ai-handoff.md`，恢复项目级首读视图。
2. 再读取 `feature-memory/index.md`，默认只展示 `ACTIVE` 和 `VERIFYING` 记忆。
3. 如果 `index.md` 缺失，扫描 `feature-memory/` 下一级 feature 目录作为候选，并提示 index 缺失。
4. 只展示 feature 摘要，不一次性展开所有文档。
5. 用户选择后读取目标 feature 的 `handoff.md`、`progress.md`、必要时读取 `design.md`、`decision.md`。
6. 汇总当前目标、状态、下一步、阻塞、风险和待确认问题。
...
6. 创建文档时读取 `references/templates.md`，并同步更新 `feature-memory/index.md` 和必要的 `feature-memory/ai-handoff.md`。
...
首读入口、记忆状态、默认读取范围、小改动跳过、单次更新限额、文件职责边界和归档压缩规则见 `references/memory-lifecycle.md`。
...
- 如果需要更新，默认选择最关键的 1-3 个 memory 文件，避免为一次提交同步大量重复信息。
...
2. 如果目录不存在，按最小结构初始化。
3. 优先读取 `feature-memory/ai-handoff.md` 作为项目级首读入口。
4. 判断任务类型，按“路由总则”读取必要文档。
5. 默认只读取 `ACTIVE` 和 `VERIFYING` 记忆；归档内容仅在命中关键词或用户要求时读取。
6. 恢复项目级背景和目标 feature 状态。
7. 对照当前代码验证关键事实，把不确定项写入“假设”或“待确认问题”。
8. 向用户简短确认当前目标、下一步动作和风险。
9. 开始开发、排障或文档更新。
...
- `ai-handoff.md`：当前活跃 feature、验证中 Bug、风险和下一步建议是否准确。
- `handoff.md`：新的 Agent 是否能在 5 分钟内恢复上下文。
- `progress.md`：关键阶段、状态变化、阻塞项和下一步是否准确，避免复制 `handoff.md` 的完整结论。
- `design.md`：设计、接口、数据结构或边界是否变化。
- `decision.md`：是否产生新的重要设计决策或取舍。
- `bug-index.md`：Bug 结束后是否登记根因分类、关键词和入口文档。

默认只更新最关键的 1-3 个 memory 文件。完成且稳定的 feature 应按 `references/memory-lifecycle.md` 归档压缩。
